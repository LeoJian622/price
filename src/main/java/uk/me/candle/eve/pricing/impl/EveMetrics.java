 ...
 * @author Candle
 */
public class EveMetrics extends AbstractPricing {
    private static final Logger logger = Logger.getLogger(EveMetrics.class);
    int batchSize = 10;
    int failureCount = 0;
    ...
    private double extractPrice(Document d, int typeID, PricingType type, PricingNumber number) {
        ...
        xPathBuilder.append("/evemetrics/type[@id=\"");
        xPathBuilder.append(typeID);
        xPathBuilder.append("\"]/");
        if (getPricingOptions().getRegions().isEmpty()) {
            xPathBuilder.append("global/");
        } else {
            xPathBuilder.append("region[@id=\"");
            xPathBuilder.append(getPricingOptions().getRegions().get(0));
            xPathBuilder.append("\"]/");
        }
        switch (number) {
        case BUY:
            xPathBuilder.append("buy/");
            break;
        default:
        case SELL:
            xPathBuilder.append("sell/");
            break;
        }
        switch (type) {
        case HIGH:
            xPathBuilder.append("maximum");
            break;
        case LOW:
            xPathBuilder.append("minimum");
            break;
        case MEAN:
            xPathBuilder.append("average");
            break;
        default:
        case MEDIAN:
            xPathBuilder.append("median");
            break;
        }
    ...
    protected Map<Integer, PriceContainer> fetchPrices(Collection<Integer> itemIDs) {
        ...
        try {
            ...
            urlBuilder.append("http://www.eve-metrics.com/api/item.xml?type_ids=");
            boolean comma = false;
            for (int i : itemIDs) {
            ...
            if (getPricingOptions().getRegions().size() > 0) {
                urlBuilder.append("&region_ids=");
                comma = false;
                for (Long regionID : getPricingOptions().getRegions()) {
                    if (comma) urlBuilder.append(",");
                    urlBuilder.append(regionID);
                    comma = true;
                    break; // XXX we're only handling the first region for now.
                }
            }
